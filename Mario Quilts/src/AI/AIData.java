...
public class AIData {
	...
	//Output the contents of scene[][] converted to characters to "text_output.txt"
	public void convertSceneFromGeneration(int[][] scene) throws IOException{
		BufferedWriter outputWriter = new BufferedWriter(new FileWriter("text_output.txt"));
			...
			for (int x = 0; x < scene[0].length; x++){
				switch(scene[y][x]){
					...
					case Sprite.ID_END:
						...
						break;
					case Sprite.ID_AIR: //If air
						outputWriter.write('O');
						break;
					case Sprite.ID_FACE_YELLOW: //If brick
					case Sprite.ID_FACE_BROWN:
					case Sprite.ID_MUSIC_NOTE:
					case Sprite.ID_CLOUD:
					case Sprite.ID_ROCK:
						outputWriter.write('B');
						break;
					case Sprite.ID_QUESTION_BLOCK: //If question block
						outputWriter.write('?');
						break;
					case Sprite.ID_PIPE_HORIZ_BL: //If pipe
					case Sprite.ID_PIPE_HORIZ_BM:
					case Sprite.ID_PIPE_HORIZ_BR:
					case Sprite.ID_PIPE_HORIZ_TL:
					case Sprite.ID_PIPE_HORIZ_TM:
					case Sprite.ID_PIPE_HORIZ_TR:
					...
					case Sprite.ID_PIPE_VERTI_TR:
						...
						break;
					case Sprite.ID_GRASS_BL: //If grass
